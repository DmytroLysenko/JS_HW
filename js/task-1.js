...
categories.forEach((el) => {
  const nameCategory = el.querySelector("h2").textContent;
  const countCategoryItems = el.querySelectorAll("ul>li").length;
  console.log(
    `Категория "${nameCategory}" содержит ${countCategoryItems} элемента(ов)`
  );
});
