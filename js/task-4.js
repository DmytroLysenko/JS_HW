...
let count;
do {
  count = prompt("Введите количество ремонтных дроидов:");
  if (count === null) {
    console.log("Отменено пользователем");
    break;
  }
  if (isNaN(Number(count)) || count < 1 || !Number.isInteger(Number(count))) {
    console.log("Введите целое число больше одного, либо нажмите отмена");
    continue;
  } else {
    console.log(
      credits < count * pricePerDroid
        ? "Недостаточно средств на счету!"
        : `Вы купили ${count} дроидов, на счету осталось ${credits -
            count * pricePerDroid} кредитов.`
    );
    break;
  }
} while (count !== null);
